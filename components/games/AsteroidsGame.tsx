...
class Ship {
  ...
  update(dt: number, keys: Record<string, boolean>) {
    ...
    this.reversing = !this.thrusting && !!keys.ArrowDown;
    if (this.reversing) {
      this.vx -= Math.cos(this.angle) * REVERSE_THRUST * dt;
      this.vy -= Math.sin(this.angle) * REVERSE_THRUST * dt;
    }
  ...
  draw(ctx: CanvasRenderingContext2D) {
    ...
    if (this.reversing && Math.random() > 0.35) {
      ctx.beginPath();
      ctx.moveTo(16, -5);
