...
export default function SalonDeLaFamaClient({
  games,
  scoresByGame,
}: {
  games: Game[];
  ...
  const youRow = user
    ? rows.find(
        (r) => r.name.trim().toLowerCase() === user.name.trim().toLowerCase(),
      )
    : undefined;
  ...
  return (
    <div className="av-hall fade-in">
      ...
      {rows.length === 0 ? (
        <div style={{ textAlign: "center", padding: "64px 12px" }}>
          <div
            className="pixel"
          ...
          <div style={{ color: "var(--ink-faint)" }}>
            Sé el primero en jugar {game.title}.
          </div>
        </div>
      ) : (
        <>
